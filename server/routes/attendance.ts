...
router.get('/:studentId', authenticate, (req: any, res) => {
  try {
...
router.post('/', authenticate, authorize(['admin']), (req, res) => {
  try {
    ...
    const existing = db.prepare('SELECT id FROM attendance_records WHERE studentId = ? AND date = ?').get(studentId, date) as any;
    ...
    const allRecords = db.prepare('SELECT status FROM attendance_records WHERE studentId = ?').all(studentId) as any[];
...
router.post('/export-and-send', authenticate, authorize(['admin']), (req, res) => {
  try {
    const { date } = req.body;
    const records = db.prepare('SELECT s.name, s.rollNumber, a.status FROM attendance_records a JOIN students s ON a.studentId = s.id WHERE a.date = ?').all(date) as any[];
