...
router.post('/', authenticate, async (req: any, res) => {
  try {
    ...
    if (receiverId === 'all') {
      ...
      const parentEmails = db.prepare('SELECT parentEmail FROM students WHERE parentEmail IS NOT NULL').all() as any[];
      parentEmails.forEach(p => {
        if (p.parentEmail) sendNotification(p.parentEmail, message);
      });
      ...
      if (receiverId.startsWith('parent_')) {
        ...
        const student = db.prepare('SELECT name, parentEmail FROM students WHERE id = ?').get(studentId) as any;
        if (student) {
           finalMessage = `[To Parent of ${student.name}]: ${message}`;
           if (student.parentEmail) sendNotification(student.parentEmail, message);
        }
        ...
        if (student) finalMessage = `[To Student ${student.name}]: ${message}`;
      }
...
router.put('/:id', authenticate, async (req: any, res) => {
  try {
    ...
    const msg = db.prepare('SELECT senderId FROM messages WHERE id = ?').get(id) as any;
